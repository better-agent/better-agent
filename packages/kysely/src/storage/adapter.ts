...
import { createKyselyOrm } from "../orm";
...
const kyselySchema = selectBetterAgentSchema();

type KyselySchema = typeof kyselySchema;
type KyselyModelName = ModelName<KyselySchema>;
type KyselyOrm = OrmClient<KyselySchema>;
...
async function upsertRow<TModelName extends KyselyModelName, TRow>(input: {
    model: ModelClient<KyselySchema, TModelName>;
    keyField: string;
    id: string;
    row: TRow;
}): Promise<void> {
...
function createTableAdapter<TModelName extends KyselyModelName, TRecord, TRow>(input: {
    model: ModelClient<KyselySchema, TModelName>;
    keyField: string;
...
function resolveAdapter(adapters: ReadonlyMap<string, TableAdapter>, table: string): TableAdapter {
    return adapters.get(table) ?? throwUnsupportedStorageTable(table, "kysely storage");
}

function requireModel<TModelName extends KyselyModelName>(
    orm: KyselyOrm,
    modelName: TModelName,
): ModelClient<KyselySchema, TModelName> | undefined {
    return orm[modelName];
}

export function kyselyStorage(options: KyselyOptions): BetterAgentStorage {
    const orm = createKyselyOrm(kyselySchema, options) as KyselyOrm;
