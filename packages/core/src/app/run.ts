...
import type { AgentState } from "../ag-ui";
...
>(
    ...
    >(
        ...
        input: AppRunInput<
            ...
            TOutput
        >,
    ) => Promise<RunResult<TState, StructuredOutputForAgent<AgentByName<TAgents, TName>, TOutput>>>;
    runResolved: <
        TName extends TAgents[number]["name"],
        TState = AgentState,
        TOutput extends AgentOutput | undefined = undefined,
            ...
            if (storage) {
                ...
                await tolerateUnsupportedStorageTable(() =>
                    storage.set(storageTables.runs, runId, {
                        ...runBase,
                        status: result.outcome === "interrupt" ? "interrupted" : "completed",
    ...
    >(
        ...
        const agent = context.config.agents.find(
        ...
        return runResolved(agent, input);
    }

    return { run: run, runResolved: runResolved };
};
