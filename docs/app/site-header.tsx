...
export default function SiteHeader() {
    ...
    useEffect(() => {
        fetch("https://api.github.com/repos/better-agent/better-agent", {
            ...
            >
                <div className="site-header-inner">
                    <Link href="/" className="site-header-logo">
                        ...
                        <span className="site-header-descriptor">Agent Framework</span>
                    </Link>
                    ...
                    <div className="site-header-actions">
                            ...
                            {navItems.map((item) => {
                                ...
                                return (
                                    ...
                                    >
                                        {item.label}
                                        {isActive && (
                                            <span className="site-header-link-indicator" />
                                        )}
                                    </Link>
                                );
                            })}
                        ...
                        >
                            <GitHubIcon className="size-[11px]" />
                            <StarIcon className="site-header-star-icon size-[9px]" />
                            {stars !== null && (
            ...
            {mobileOpen && (
                <>
                    <button
                        ...
                        className="site-header-overlay md:hidden"
                        onClick={() => setMobileOpen(false)}
                    />
                    <nav className="site-header-mobile md:hidden">
                        {navItems.map((item) => {
                            const isActive =
                                item.href === "/"
                                    ? pathname === "/"
                            ...
                            return (
                                <Link
                                    ...
                                    href={item.href}
                                    onClick={() => setMobileOpen(false)}
                                    className={`site-header-mobile-link ${isActive ? "active" : ""}`}
                                >
                                    {item.label}
                                </Link>
                            );
