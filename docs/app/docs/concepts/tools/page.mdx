...
<Callout type="info">
  Client tools requires [Storage](/docs/concepts/storage) to be configured for
  runs to store interrupted runs and resume later.
</Callout>
...
Client tool pauses use [AG-UI](https://docs.ag-ui.com) interrupts. Each interrupt has an id and response
