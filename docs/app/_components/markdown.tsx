...
function CodeBlock({
    ...
    const formattedCode = useMemo(() => {
        if (["ts", "tsx", "typescript", "js", "javascript", "json"].includes(displayLang)) {
    ...
    return (
        <div style={{ tabSize: 2 }} className="my-4">
            <DynamicCodeBlock
                lang={displayLang}
                code={formattedCode}
                options={{
...
export function Markdown({ text }: { text: string }) {
    ...
    return (
        <Suspense fallback={<p className="invisible whitespace-pre-wrap">{text}</p>}>
